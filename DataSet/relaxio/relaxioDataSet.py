...
#specficWebsite1 = df[df[0].str.contains("../../Dataset/Train/net.relaxio.relaxio")]
#specficWebsite1 = specficWebsite1[0].str.split("	", expand=True)
#specficWebsite1 = specficWebsite1[specficWebsite1.index != 128189]



specficWebsite1 = pd.read_csv('./DataSet/relaxio/Modifiedrelaxio.csv')
...
#specficWebsite1.to_csv('./DataSet/relaxio/Modifiedrelaxio.csv')
qasdict = dict()
qasdict['qas'] = qas
qasdict['context'] = aa
...
with open('./DataSet/relaxio/data.json', 'w') as f:
    json.dump(qasdict, f)
...
def find_index(text1):
    with open('./DataSet/relaxio/relaxioPrivacyPage.txt', 'r',encoding="utf8") as f:
        content = f.read()
