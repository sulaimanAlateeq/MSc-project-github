...
specficWebsite1 = df[df[0].str.contains("../../Dataset/Train/com.usps")]
specficWebsite1 = specficWebsite1[0].str.split("	", expand=True)
#specficWebsite1 = specficWebsite1[specficWebsite1.index != 128189]



specficWebsite1 = pd.read_csv('./DataSet/usps/Modifiedusps.csv')
...
#specficWebsite1.to_csv('./DataSet/usps/Modifiedusps.csv')
qasdict = dict()
qasdict['qas'] = qas
qasdict['context'] = aa
...
with open('./DataSet/usps/data.json', 'w') as f:
    json.dump(qasdict, f)
...
def find_index(text1):
    with open('./DataSet/usps/uspsPrivacyPage.txt', 'r',encoding="utf8") as f:
        content = f.read()
