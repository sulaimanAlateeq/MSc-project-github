...
#specficWebsite = df[df[0].str.contains("../../Dataset/Train/com.shutterfly")]
...
specficWebsite1 = pd.read_csv('./DataSet/shutterfly/Modifiedshutterfly.csv')
...
#SpecficWebsite1.to_csv('./DataSet/shutterfly/Modifiedshutterfly.csv')
qasdict = dict()
qasdict['qas'] = qas
qasdict['context'] = aa
...
with open('./DataSet/shutterfly/data.json', 'w') as f:
    json.dump(qasdict, f)
...
def find_index(text1):
    with open('./DataSet/shutterfly/shutterflyPrivacyPage.txt', 'r',encoding="utf8") as f:
        content = f.read()
