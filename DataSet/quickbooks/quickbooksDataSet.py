...
#specficWebsite = df[df[0].str.contains("../../Dataset/Train/com.intuit.quickbooks")]
#specficWebsite1 = specficWebsite[0].str.split("	", expand=True)

specificWebiste1 = pd.read_csv('./DataSet/quickbooks/Modifiedquickbooks.csv')
specificWebiste1 = specificWebiste1.drop('11',1)

qas = []
r = 1
for index, row in specificWebiste1.iterrows():    
    dicn = dict()
    answers = dict()
    dicn['question']=row[5]
    dicn['id']= int(str(210000) + str(r))
    r = r+1
    answers['text'] = row[6]
...
#specficWebsite1.to_csv('./DataSet/quickbooks/Modifiedquickbooks.csv')
qasdict = dict()
qasdict['qas'] = qas
qasdict['context'] = aa
...
with open('./DataSet/quickbooks/data.json', 'w') as f:
    json.dump(qasdict, f)
...
def find_index(text1):
    with open('./DataSet/quickbooks/quickbooksPrivacyPage.txt', 'r',encoding="utf8") as f:
        content = f.read()
