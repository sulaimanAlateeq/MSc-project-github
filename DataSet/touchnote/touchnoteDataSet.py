...
#specficWebsite1 = df[df[0].str.contains("../../Dataset/Train/com.touchnote.android")]
#specficWebsite1 = specficWebsite1[0].str.split("	", expand=True)



specficWebsite1 = pd.read_csv('./DataSet/touchnote/Modifiedtouchnote.csv')
...
#specficWebsite1.to_csv('./DataSet/touchnote/Modifiedtouchnote.csv')
qasdict = dict()
qasdict['qas'] = qas
qasdict['context'] = aa
...
with open('./DataSet/touchnote/data.json', 'w') as f:
    json.dump(qasdict, f)
...
def find_index(text1):
    with open('./DataSet/touchnote/touchnotePrivacyPage.txt', 'r',encoding="utf8") as f:
        content = f.read()
